function [hess,err] = hessian(fun,x0,h)
...
% My addition
p=length(x0);stepsize=h.*ones(1,p);
...
for i = 2:nx
    for j = 1:(i-1)
        ...
        for k = 1:(params.RombergTerms+1)
            ...
            %My addition
            H=dfac(k)*h;
            X1=x0 + swap2(zeros(sx),i,H,j,H);
            X2=x0 + swap2(zeros(sx),i,-H,j,-H);
            X3=x0 + swap2(zeros(sx),i,H,j,-H);
            X4=x0 + swap2(zeros(sx),i,-H,j,H);
            
            A1=Correct(fun,X1,h,p);
            A2=Correct(fun,X2,h,p);
            A3=Correct(fun,X3,h,p);
            A4=Correct(fun,X4,h,p);
            %*************************************************************
            dij(k)=A1+A2-A3-A4;
        end
...
%*************************************************************
%My addition
function f=Correct(fun,x,h,p)
f=fun(x);
while isequal(f,realmax)
    x0=x+10^(-3).*h.*randn(1,p);
    f=fun(x0);
end
end
%*************************************************************
