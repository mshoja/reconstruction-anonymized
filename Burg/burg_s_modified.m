function [rc,var,ASAcontrol] = burg_s_modified(f,b,max_order,last)
...
%
% NOTE: Since mode=1 is numerically unstable, I (JSE) modified the code
% such that always mode=3 is used instead of mode=1 (line 208). Also line
% 276 is modified, because it is also unstable.
...
% Always use mode=3, because mode=1 is numerically unstable (JSE, 13-2-2012)
protocol = [0 3 0;0 0 0;0 0 0;0 P 0];
p = 0;
p_max = 0;
i2 = 2;
...
%            sum((a(a3(c1),:,c1)+rc(i1)*v(v2(c1),:,c1)).^2);
         den = sum(sum(v(v1(c0):v2(c0),:,c0).*...
            v(v1(c0):v2(c0),:,c0),i9),i10) + ...
            sum(sum(a(a2(c0):a3(c0),:,c0).*...
            a(a2(c0):a3(c0),:,c0),i9),i10);
...
if state %f and b have been provided as input 
      ...
      if isempty(b)
         rc = [1 rc(2:end)];%rc(2:end);
      else
         rc = [1 rc(3:end)];%rc(3:end);
      end
else
   rc(1) = 1;
end
