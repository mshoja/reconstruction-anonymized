function [cic,pe_est] = cic_s(res,n_obs,Keff_model,Keff_mean,cand_order,last)
%CIC Finite sample order selection criterion
%   Adapted for use with segments.
...
case 3
   if isa(Keff_model,'struct'), error(ASAerr(2,mfilename))
   end
   cand_order=[]; ASAcontrol=[];
case 4
   if isa(Keff_mean,'struct'), error(ASAerr(2,mfilename))
   end
   cand_order=[]; ASAcontrol=[];
case 5 
...
case 'burg'
   vi = 1./(n_obs+1-i)/Keff_model;
   vi(1)=1/n_obs/Keff_mean;
otherwise
...
% Always use finite sample equations for PE
pe_est = res.*(fsic_tail+1);
% Use finite-sample equations for CIC only when Keff_model==1
% For the paper I will use GIC always, to compare differences in order
% selection criteria with 1 segment.
%if Keff_model==1
     cic_tail = max(fic_tail,fsic_tail);
     cic = log(res)+cic_tail;
%else%GIC with penalty factor 2.7
%    penalty_factor=2.7;
%    cic=log(res)+penalty_factor*(0:max_order)/n_obs/Keff_model+vi(1);
%end

    %pe_est=res.*(1+(0:max_order)/n_obs/Keff_model+vi(1))./(1-(0:max_order)/n_obs/Keff_model-vi(1));
% end
