function [ar,ma,ASAsellog,ASAcontrol]=armasel_s(sig,cand_ar_order,cand_ma_order,cand_arma_order,arma_order_diff,last,estKeff_model,estKeff_mean)
...
%   Keff(model) here, or use a value chosen by the user.
...
%-------------------------------------------------------------------------
% estKeff_model=[a b]. If a>0, we use that as effective number of segments,
% otherwise we estimate it. b is the line separation. We assume the same
% correlation function in x and y directions. But we have to subsample the
% correlation function in case b>1.
if estKeff_model(1)>0
    Keff_model=max(estKeff_model,1);
    Keff_mean=max(estKeff_mean,1);
elseif n_segs>1
   S=estKeff_model(2);
   % We need |cor|^2 of the data, up to lag b*(n_segs-1)
   % Orders selected per segment
   %arorders=zeros(1,n_segs);
   Pmax=S*(n_segs-1);
   % Estimated RCs per segment
   ...
   for k=1:n_segs
       ...
       cor=arma2cor(ar,ma,(n_segs-1)*S);
       % Average with gain
       COR(k,:)=std(sig(:,k))^2*cor(1:S:end);
   end
   % Necessary to reset this global variable !!
   ASAglob_rc=[];
   COR=mean(COR);COR=COR/COR(1);
   t=2:n_segs;
   Keff_model=n_segs/(1+2*sum((1-(t-1)/n_segs).*COR(t).^2));
...
[ar_ar,ar_sellog] = sig2ar_s(sig,cand_ar_order,ASAcontrol,Keff_model,Keff_mean);
[ma_ma,ma_sellog] = sig2ma_s(sig,cand_ma_order,ASAcontrol,Keff_model,Keff_mean);
[arma_ar,arma_ma,arma_sellog] = sig2arma_s(sig,cand_arma_order,arma_order_diff,ASAcontrol,Keff_model,Keff_mean);
