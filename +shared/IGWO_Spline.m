...
function [par, Alpha_score] = IGWO_Spline(varargin)
    ...
    % Initialization
    index_sigma = [false(1,M) true(1,dim-M)];
    if isempty(LP)
        Positions = shared.LegitimateRegion(fobj, dim, lb, ub, N, index_sigma);

        fprintf('\n');
        disp(['At least ' num2str(size(Positions,1)) ' initial legitimate solutions were successfully found']);
        fprintf('\n');
        Positions = sortrows(Positions, dim+1);
        Positions = Positions(1:N, 1:dim);
    else
        Positions = LP(1:N,1:end-1);
    end
    LB = repmat(lb, [N 1]);
    UB = repmat(ub, [N 1]);
    %***********************************************************
    ...
    while iter < Max_iter && ~stop
        for i = 1:N
            ...
            % IDX = X_GWO(:, M + 1:end) < LB;
            % IDX = [false(N, M) IDX];
            % X_GWO(IDX) = (Positions(IDX) + LB(IDX)) ./ 2; 
            % 
            % IDX = X_GWO(:, M + 1:end) > UB;
            % IDX = [false(N, M) IDX];
            % X_GWO(IDX) = (Positions(IDX) + LB(IDX)) ./ 2; 
